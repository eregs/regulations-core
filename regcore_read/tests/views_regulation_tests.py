...
class ViewsRegulationTest(TestCase):
    @patch('regcore_read.views.regulation.storage')
    def test_get_good(self, storage):
        url = '/regulation/lab/ver'
        storage.for_regulations.get.return_value = {"some": "thing"}
        response = Client().get(url)
        self.assertTrue(storage.for_regulations.get.called)
        args = storage.for_regulations.get.call_args[0]
    ...
    def test_get_empty(self, storage):
        url = '/regulation/lab/ver'
        storage.for_regulations.get.return_value = {}
        response = Client().get(url)
        self.assertTrue(storage.for_regulations.get.called)
        args = storage.for_regulations.get.call_args[0]
        self.assertIn('lab', args)
    ...
    def test_listing(self, storage):
        ...
        storage.for_regulations.listing.return_value = [
